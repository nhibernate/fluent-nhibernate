...
{
    public class ClassMapOldx<T> : MapBase<T>, IMappingProvider
    {
        ...
        protected ClassMapOldx(ClassMapping classMapping)
            : base(classMapping)
        {
        ...
        public ManyToManyPartOld<T, CHILD> HasManyToMany<CHILD>(Expression<Func<T, object>> expression)
        {
            PropertyInfo info = ReflectionHelper.GetProperty(expression);

            var part = new ManyToManyPartOld<T, CHILD>(info);
            _deferredCollections.Add(part);
            return part;
        }
        ...
        public DiscriminatorPartOld DiscriminateSubClassesOnColumn(string columnName)
        {
            var mapping = new DiscriminatorMapping { ColumnName = columnName };
            _classMapping.Discriminator = mapping;
            return new DiscriminatorPartOld(mapping);
        }
